-- =============================================================================
-- AUTOSERVICE CRM — полная миграция PostgreSQL (001_init.sql … 010_payout_period.sql)
-- =============================================================================
...
-- 2) Блок в конце создаёт таблицу migrations и помечает 001–010 как применённые —
--    иначе при старте приложение попытается выполнить те же ALTER повторно.
-- 3) На Railway обычно хватает переменной DATABASE_URL: applyMigrations() при boot.
