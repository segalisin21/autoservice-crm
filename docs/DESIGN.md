...
- Перенос записи: drag-and-drop карточки в другую колонку/час или выпадающий список мастера на карточке → `PATCH /orders/:id/schedule` (обновляет `assigned_user_id`, при необходимости время, и `master_id` на work-строках).
...
| Сотрудник | Да |
| Комментарий | Нет |
| Окончание, тип работ, марка, владелец | Нет (accordion «Дополнительно») |
