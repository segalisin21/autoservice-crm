...
## 15. Автосервис CRM — UX-потоки (2026-06)

### Главная
1. Календарь месяца или расписание дня — первый блок после входа.
2. Компактные KPI дня (заказы, сумма, занятость).
3. Быстрый поиск по госномеру — внизу страницы.

### Расписание дня (`mode=day`)
- Колонки — **мастера** и сотрудники с флагом «Колонка в расписании» (`show_in_schedule`, у Виталика включён по умолчанию); менеджеры без флага не показываются.
- Порядок колонок: `users.schedule_order`; в заголовке колонки стрелки ← → (роли с `orders:mutate`) меняют порядок с соседом.
- Перенос записи: drag-and-drop карточки в другую колонку/час → `PATCH /orders/:id/schedule`.
- Заказ, назначенный на не-мастера, отображается в блоке «Без сотрудника».

### Госномер
- Поля `plate-search-input`: ввод букв и цифр подряд; клиент собирает формат `А123ВС777`.
- Сервер валидирует полный номер или пустое значение.

### Марка / модель
- Autocomplete из справочника (`vehicle_marks`, `vehicle_models`); sync с Auto.ru — кнопка в каталоге или `npm run sync:vehicles`.
- При выборе модели подставляются год и тип кузова.

### Прайс / услуги в заказе
- Каталог: карточки или таблица; **работы** — полная форма с tier-ценами; **товары** — отдельная карточка (`/catalog/products/new`: название, артикул, закуп, цена продажи).
- В заказе: подсказки из каталога с автоподстановкой цены.

### Создание заказа — `/orders/new`

```mermaid
flowchart TD
  plateSearch[Поиск по госномеру] --> found{Найдено?}
  found -->|да| selectCar[Выбор авто]
  found -->|нет| plateOnly[Госномер в форме]
  selectCar --> minimal[Дата + время + сотрудник]
  plateOnly --> minimal
  minimal --> optional[Опционально: комментарий, тип работ, авто/владелец]
  optional --> create[POST /orders]
  create --> card[Карточка заказа]
  card --> intake[Дозаполнение при приёмке]
```

| Поле | Обязательно при создании |
| --- | --- |
| Госномер / авто из базы | Нет (можно при приёмке) |
| Дата | Да |
| Время начала | Да |
| Сотрудник | Да |
| Окончание | Нет (основная форма; для блока в расписании) |
| Дата окончания | Нет (если машина на несколько дней) |
| Комментарий | Нет |
| Тип работ, марка, владелец, VIN | Нет (accordion «Дополнительно» или при приёмке) |

- Заказ без авто создаётся с `car_id = NULL`; на карточке — блок «Авто и клиент» (`POST /orders/:id/car`).
- Ошибки 400: «Укажите дату», «Укажите время начала», «Выберите сотрудника», «Укажите госномер или выберите автомобиль из базы».

### Исправления баг-пула (2026-06-18)

#### Работы: несколько мастеров на одну строку
- При добавлении работы чекбоксы «Мастера» создают **одну** строку в таблице работ с полной суммой для клиента.
- В колонке «Мастер» — список имён через запятую (напр. «Артур, Виталик»).
- Редактирование строки: те же чекбоксы; до статуса `completed` состав мастеров можно менять.
- ЗП: доля каждого мастера через `order_line_payroll.share_percent` (равное деление 50/50 при двух мастерах). Клиентская сумма **не** делится.
- При закрытии заказа начисления фиксируются в строках; смена правила мастера сама историю не меняет. Ручной пересчёт с даты — `/admin/payroll` → «Пересчёт начислений».
- Мастер в своём кабинете видит строку, если он в payroll, даже если не первый в списке.

#### VIN при приёмке
- Форма «Авто и клиент» на карточке заказа: поля VIN, год, кузов (как при создании заказа).
- VIN опционален; сохраняется uppercase. Отображается в шапке заказа и в заказ-наряде.

#### Блок расписания по времени
- «Окончание» вынесено на основную форму создания заказа (рядом с «Начало»); подсказка про блок в расписании.
- Без `end_time` карточка занимает 1 час; с `end_time` — `rowspan` на N часов (desktop ≥1280px).
- При **пересечении** заказов у одного мастера — до N **полос** (lanes) рядом в колонке; `rowspan` считается **отдельно в каждой полосе** (длинный заказ 10:00–13:00 растягивается, короткий в соседней полосе — одна строка).
- Mobile (`garage-day-grid`): диапазон «10:00–13:00» в тексте карточки.
- Валидация: `end_time` > `start_time`.

#### Многодневный заезд
- Поле «Дата окончания» (`scheduled_end_date`) — опционально, ≥ даты начала.
- В расписании заказ виден **только в день начала**; badge «до ДД.ММ» если end > start.
- Печать: строка «Дата окончания работ» при многодневном заезде.
