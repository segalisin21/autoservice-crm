...
- RBAC: **owner / admin / manager / master**.
...
- `admin`: операционка + финансы/расходы/ЗП/аналитика (без управления пользователями и настройками).
- `manager`: календарь, заказы (создание и редактирование), клиенты, авто, оплаты; без журнала, финансов и админки.
- `master`: календарь, карточка заказа (просмотр своих работ), комментарий и фото, своя ЗП; без списка заказов и мутаций работ.
...
- `orders:view`, `orders:mutate`, `orders:annotate`
...
### Дефолтная матрица
- `owner`: всё.
- `admin`: операционка + каталог + финансы/расходы/ЗП/аналитика.
- `manager`: `dashboard:view`, `clients:*`, `cars:*`, `orders:view`, `orders:mutate`, `catalog:view`, `payments:*`.
- `master`: `dashboard:view`, `orders:view`, `orders:annotate`, `payroll:view` (только своё); список `/orders` редиректит на календарь.
