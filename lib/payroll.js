...
async function resolveCompRule(db, masterId, catalogItemId, effectiveDate, fallback = null) {
  ...
  if (catalogItemId) {
    ...
    const catalogRows = await db.query(
      `
      SELECT payroll_fixed FROM catalog_items
      WHERE id = ? AND type = 'work' AND payroll_fixed IS NOT NULL
      LIMIT 1
    `,
      [catalogItemId]
    );
    if (catalogRows[0]) {
      return { mode: "fixed", value: Number(catalogRows[0].payroll_fixed) || 0 };
    }
  }
...
/**
 * Consumables that reduce payroll base (net_percent): only cost_price on the work line.
 * Product-tab lines are excluded — they affect P&L, not master earned.
 */
function payrollMaterialsForWorkLine(line) {
  return lineConsumableCost(line);
}

/**
 * Computes the master's earned amount for a single work line.
 * For mode "net_percent" payrollMaterialsForWorkLine is subtracted before the percentage.
 */
function computeEarnedForLine(line, rule, opts = {}) {
  ...
  if (mode === "fixed") {
    const share = Number(opts.share);
    const factor = Number.isFinite(share) && share > 0 ? share : 1;
    return { mode, value, earned: round2(value * factor) };
  }
  if (mode === "hourly") {
  ...
  for (const line of lines) {
    ...
    if (payrollRows.length) {
      for (const pr of payrollRows) {
        ...
        const matShare = round2(lineMaterials * share);
        const { earned } = computeEarnedForLine(virtualLine, rule, {
    ...
    const { earned } = computeEarnedForLine(line, rule, { allocatedMaterials: lineMaterials });
    total += earned;
  }
...
async function freezeOrderEarned(orderId, opts = {}) {
  ...
  for (const line of lines) {
    ...
    if (payrollRows.length) {
      let sumEarned = 0;
      for (const pr of payrollRows) {
        if (!force && pr.earned_amount != null) {
      ...
      if (force || line.master_comp_mode == null) {
        await db.query(
          `
    ...
    if (!line.master_id) continue;
    const rule = await resolveCompRule(db, line.master_id, line.catalog_item_id, effectiveDate, fallback);
    const { mode, value, earned } = computeEarnedForLine(line, rule, { allocatedMaterials: lineMaterials });
    await db.query(
      `
      UPDATE order_lines SET
        master_comp_mode = ?,
...
async function recalculatePayrollFromDate(db, { from_date }) {
  const date = String(from_date ?? "").trim().slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    return { ordersProcessed: 0, error: "invalid_date" };
  }

  const orders = await db.query(
    `
    SELECT id FROM orders
    WHERE status = 'completed' AND closed_at >= ?
    ORDER BY id
  `,
    [`${date} 00:00:00`]
  );
  ...
  for (const row of orders) {
    await freezeOrderEarned(row.id, { force: true });
  }
