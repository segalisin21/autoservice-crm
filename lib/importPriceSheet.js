...
async function upsertCatalogItem(db, item) {
  ...
  if (existing[0]) {
    await db.query(
      ...
      UPDATE catalog_items SET
        ...
        description = ?, default_material_cost = COALESCE(?, default_material_cost), is_active = 1
      WHERE id = ?
    `,
      [
        item.default_price,
        ...
        item.price_tier_3,
        item.description,
        item.default_material_cost ?? null,
        existing[0].id
      ]
    );
  ...
  await db.query(
    ...
    INSERT INTO catalog_items(
      ...
      default_price, price_tier_2, price_tier_3, default_material_cost, unit, is_active
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'усл.', 1)
  `,
    [
      item.type,
