...
async function getMonthCalendar(year, monthIndex0) {
  ...
  const absenceDays = await loadAbsenceDaysInMonth(db, year, monthIndex0);
  ...
  for (let day = 1; day <= end.getDate(); day++) {
    ...
    days.push({
      ...
      has_absence: absenceDays.has(date)
    });
  }
...
function decorateOrderCard(row, absences) {
  const carLabel =
  ...
  const assigned_user_id = row.assigned_user_id != null ? Number(row.assigned_user_id) : null;
  return {
    id: row.id,
    status: row.status,
...
async function getDayByEmployees(dateStr) {
  ...
  for (const row of rows) {
    const card = decorateOrderCard(row, absences);
