...
function formatEndDateBadge(scheduledDate, scheduledEndDate) {
  const start = String(scheduledDate ?? "").slice(0, 10);
  const end = String(scheduledEndDate ?? "").slice(0, 10);
...
function decorateOrderCard(row, absences, assignedOverride = null) {
  ...
  const scheduled_date = String(row.scheduled_date ?? "").slice(0, 10) || null;
  const scheduled_end_date = String(row.scheduled_end_date ?? "").slice(0, 10) || null;
  const end_date_badge = formatEndDateBadge(scheduled_date, scheduled_end_date);
...
async function getDayByEmployees(dateStr) {
  ...
  const rows = await db.query(
    ...
    SELECT o.id, o.status, o.work_type, o.total_price, o.notes,
           o.scheduled_date, o.scheduled_end_date,
...
function orderEndMinutes(startTime, endTime) {
  const startMin = parseMinutesFromTime(startTime);
  if (startMin == null) return null;
  const endMin = parseMinutesFromTime(endTime);
  if (endMin == null || endMin <= startMin) return startMin + 60;
  return endMin;
}

function orderInterval(order) {
  const startMin = parseMinutesFromTime(order.start_time);
  if (startMin == null) return null;
  const endMin = orderEndMinutes(order.start_time, order.end_time);
...
function assignLanes(orders) {
  ...
  const sortable = orders
    .map((order) => ({ order, interval: orderInterval(order) }))
    .filter((x) => x.interval != null);

  sortable.sort((a, b) => {
    if (a.interval.startMin !== b.interval.startMin) return a.interval.startMin - b.interval.startMin;
    const spanA = calcSpanRows(a.order.start_time, a.order.end_time);
    const spanB = calcSpanRows(b.order.start_time, b.order.end_time);
    return spanB - spanA;
  });

  const laneIntervals = [];

  for (const { order, interval } of sortable) {
    let placed = false;
    for (let i = 0; i < laneIntervals.length; i++) {
      const conflicts = laneIntervals[i].some((iv) => intervalsOverlap(iv, interval));
      if (!conflicts) {
        laneIntervals[i].push(interval);
        order.lane = i;
        placed = true;
        break;
      }
    ...
    if (!placed) {
      order.lane = laneIntervals.length;
      laneIntervals.push([interval]);
    }
  }

  for (const o of orders) {
    if (o.lane == null) o.lane = 0;
  }

  return Math.max(1, ...orders.map((o) => (o.lane ?? 0) + 1));
}
...
function assignOrdersToTimeSlots(dayData) {
  ...
  const columns = (dayData.employees || []).map((emp) => {
    ...
    for (const o of emp.orders || []) {
      ...
      if (h != null) timedOrders.push(o);
      else noTime.push(o);
    }

    const maxLanes = assignLanes(timedOrders);
    const lanes = [];
    ...
    for (let lane = 0; lane < maxLanes; lane++) {
      ...
      lanes.push({ byHour, coveredBySpan });
    }

    for (const o of timedOrders) {
      const laneIdx = o.lane ?? 0;
      const h = parseHourFromTime(o.start_time);
      if (h == null || !lanes[laneIdx]?.byHour[h]) continue;
      const span_rows = Math.min(calcSpanRows(o.start_time, o.end_time), SCHEDULE_END_HOUR - h + 1);
      lanes[laneIdx].byHour[h].push({ ...o, span_rows, lane: laneIdx });
      for (let offset = 1; offset < span_rows; offset++) {
        const nextHour = h + offset;
        if (lanes[laneIdx].coveredBySpan[nextHour] != null) {
          lanes[laneIdx].coveredBySpan[nextHour] = true;
        }
      }
    }
