...
const { sqlDateOf, orderDateSql, sqlMonthYmd } = require("../config/sqlDialect");
...
async function getMonthlyComparison(db, months = 12) {
  const monthExpr = sqlMonthYmd(db.dialect, "o.closed_at");
  const rows = await db.query(
    `
...
async function getMasterRevenue(db, startDate, endDate, limit = 50) {
  const rows = await db.query(
    `
    SELECT u.name, u.id AS user_id,
           COUNT(DISTINCT ol.id) AS lines_count,
           COALESCE(SUM(ol.total), 0) AS revenue,
           COALESCE(SUM(ol.master_earned_amount), 0) AS payroll
    FROM order_lines ol
  ...
  return rows.map((r) => ({
    ...
    revenue: round2(r.revenue),
    payroll: round2(r.payroll)
  }));
}

async function getOrdersWithReceivables(db, limit = 200) {
  const orders = await db.query(
    `
    SELECT o.id, o.total_price, o.status, o.scheduled_date,
           cl.full_name AS client_name, c.license_plate_raw
    FROM orders o
  ...
  for (const o of orders) {
    ...
    const due = round2(Math.max(0, Number(o.total_price) - paid));
    if (due > 0) {
      out.push({
        order_id: o.id,
        client_name: o.client_name,
        license_plate_raw: o.license_plate_raw,
        status: o.status,
