...
async function sumEarnedForUser(db, userId, { start_date, end_date } = {}) {
  ...
  if (start_date && end_date) {
    ...
    dateParams.push(`${start_date} 00:00:00`, `${end_date}T23:59:59`);
  }
    ...
    return db.query(
      ...
      SELECT * FROM (
        ...
        WHERE ol.line_type = 'work'
          ...
          AND o.status = 'completed'
          AND ol.master_earned_amount IS NOT NULL
          AND NOT EXISTS (SELECT 1 FROM order_line_payroll olp2 WHERE olp2.order_line_id = ol.id)
          ${dateFilter}
      ) AS earned_lines
      ORDER BY closed_at DESC, line_id DESC
      LIMIT ?
    `,
      [userId, ...dateParams, userId, ...dateParams, limit]
    );
  } catch {
    const params = [userId];
    if (start_date && end_date) {
