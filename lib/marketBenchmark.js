...
function buildChart(rows) {
  ...
  const ranked = [...withPrice].sort((a, b) => {
    const da = Number.isFinite(a.deltaPct) ? Math.abs(a.deltaPct) : -1;
    const db = Number.isFinite(b.deltaPct) ? Math.abs(b.deltaPct) : -1;
    if (db !== da) return db - da;
    return (b.effectivePrice || 0) - (a.effectivePrice || 0);
  });
  const slice = ranked.slice(0, CHART_MAX_ROWS);
  return {
    labels: slice.map((r) => r.name),
    ourPrices: slice.map((r) => r.effectivePrice),
    marketTypical: slice.map((r) => round2((r.typicalLow + r.typicalHigh) / 2)),
    marketMin: slice.map((r) => r.min),
    marketHigh: slice.map((r) => r.high),
    shown: slice.length,
    total: withPrice.length,
    truncated: withPrice.length > CHART_MAX_ROWS
  };
}
