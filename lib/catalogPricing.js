...
function parseOptionalTierMaterial(raw) {
  const s = String(raw ?? "").trim();
  if (!s) return null;
  return parseMoney(s.replace(/[^\d,.-]/g, "").replace(",", "."));
}

function parseOptionalPayrollFixed(raw) {
  const s = String(raw ?? "").trim();
  if (!s) return null;
  const n = parseMoney(s.replace(/[^\d,.-]/g, "").replace(",", "."));
  return n >= 0 ? n : null;
...
function materialForVehicleTier(item, tier) {
  if (!item) return 0;
  const t = normalizeVehicleTier(tier);
  if (t === 2 && item.material_cost_tier_2 != null && item.material_cost_tier_2 !== "") {
    return parseMoney(item.material_cost_tier_2);
  }
  if (t === 3 && item.material_cost_tier_3 != null && item.material_cost_tier_3 !== "") {
    return parseMoney(item.material_cost_tier_3);
  }
  return parseMoney(item.default_material_cost);
}
...
module.exports = {
  ...
  priceForVehicleTier,
  materialForVehicleTier,
  parseOptionalTierPrice,
  parseOptionalTierMaterial,
  parseOptionalPayrollFixed
};
