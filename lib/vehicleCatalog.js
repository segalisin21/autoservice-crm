function likePattern(term) {
  return `%${String(term).trim().toLowerCase()}%`;
}

async function searchMarks(db, q, limit = 15) {
  const term = String(q ?? "").trim();
  if (!term) {
    return db.query(
      "SELECT id, autoru_id, name, name_ru FROM vehicle_marks ORDER BY name_ru LIMIT ?",
      [limit]
    );
  }
  const like = likePattern(term);
  return db.query(
    `
    SELECT id, autoru_id, name, name_ru FROM vehicle_marks
    WHERE LOWER(name_ru) LIKE ? OR LOWER(name) LIKE ? OR LOWER(autoru_id) LIKE ?
    ORDER BY name_ru
    LIMIT ?
  `,
    [like, like, like, limit]
  );
}

async function searchModels(db, markId, q, limit = 15) {
  const id = Number(markId);
  if (!Number.isFinite(id) || id <= 0) return [];
  const term = String(q ?? "").trim();
  if (!term) {
    return db.query(
      `
      SELECT id, mark_id, autoru_id, name, name_ru, year_from, year_to
      FROM vehicle_models WHERE mark_id = ?
      ORDER BY name_ru LIMIT ?
    `,
      [id, limit]
    );
  }
  const like = likePattern(term);
  return db.query(
    `
    SELECT id, mark_id, autoru_id, name, name_ru, year_from, year_to
    FROM vehicle_models
    WHERE mark_id = ? AND (LOWER(name_ru) LIKE ? OR LOWER(name) LIKE ? OR LOWER(autoru_id) LIKE ?)
    ORDER BY name_ru LIMIT ?
  `,
    [id, like, like, like, limit]
  );
}

async function searchModelsGlobal(db, q, limit = 15) {
  const term = String(q ?? "").trim();
  if (!term) return [];
  const like = likePattern(term);
  return db.query(
    `
    SELECT m.id, m.mark_id, m.autoru_id, m.name, m.name_ru, m.year_from, m.year_to,
           mk.name AS mark_name, mk.name_ru AS mark_name_ru, mk.autoru_id AS mark_slug
    FROM vehicle_models m
    JOIN vehicle_marks mk ON mk.id = m.mark_id
    WHERE LOWER(m.name_ru) LIKE ? OR LOWER(m.name) LIKE ? OR LOWER(m.autoru_id) LIKE ?
       OR LOWER(mk.name_ru) LIKE ? OR LOWER(mk.name) LIKE ? OR LOWER(mk.autoru_id) LIKE ?
    ORDER BY mk.name_ru, m.name_ru
    LIMIT ?
  `,
    [like, like, like, like, like, like, limit]
  );
}

async function listGenerations(db, modelId) {
  const id = Number(modelId);
  if (!Number.isFinite(id) || id <= 0) return [];
  return db.query(
    `
    SELECT id, model_id, autoru_id, name, body_type, year_from, year_to
    FROM vehicle_generations WHERE model_id = ?
    ORDER BY year_from DESC, name
  `,
    [id]
  );
}

async function getModelById(db, modelId) {
  const rows = await db.query(
    `
    SELECT m.id, m.mark_id, m.autoru_id AS model_slug, m.name_ru AS model_name, m.year_from, m.year_to,
           mk.name_ru AS mark_name, mk.autoru_id AS mark_slug
    FROM vehicle_models m
    JOIN vehicle_marks mk ON mk.id = m.mark_id
    WHERE m.id = ?
  `,
    [modelId]
  );
  return rows[0] || null;
}

module.exports = { searchMarks, searchModels, searchModelsGlobal, listGenerations, getModelById };
