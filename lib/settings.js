...
async function loadVehicleCatalogSettings(db) {
  const raw = (await getSetting(db, "vehicle_catalog_names", "original")) || "original";
  return {
    vehicle_catalog_names: raw === "cyrillic" ? "cyrillic" : "original"
  };
}
...
async function ensureDefaultSettings(db) {
  const defaults = [
    ...
    ["vehicle_catalog_names", "original"]
  ];
