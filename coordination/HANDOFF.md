...
## 2026-09-02 — анализ рынка под реальные услуги CRM

### What changed
- **`/admin/market` пересобран:** вместо ремней/перетяжки — 8 направлений CRM (мойка, химчистка, защита ЛКП, автоэлектрика, допоборудование, шумоизоляция, автозвук, перетяжка) и 3 географии (Чебоксары, близлежащие, Казань/крупные).
- **Наши цены живые:** подтягиваются из `catalog_items` по `article`; для услуг вне каталога (шумка, автозвук, перетяжка) — факт из `order_lines` по `name_lc`.
- **Новые KPI:** сопоставлено услуг, ниже рынка, выше рынка, продаём без каталога, медианное отклонение.
- **Новые блоки:** «Продаём, но нет в каталоге», «Ниши без нашего предложения», «Рекомендации по цене», источники среза.
- **График:** горизонтальный bar «наша цена против рынка» (`#marketPositionChart`) вместо графика цен на ремни.
- **График (фикс отображения):** не более 12 услуг с наибольшим отклонением; высота контейнера по числу строк; подписи обрезаются, полное имя — в tooltip; при усечении — подсказка «N из M».
- **Интерактивность `/admin/market`:** панель графика (тип: цены / отклонение % / распределение; сортировка; лимит 6–20 / все; фильтр позиции); кликабельные KPI-карточки; клик по строке таблицы — фокус на услуге; сортировка таблицы по колонкам; клик по сектору donut — фильтр позиции; настройки графика в URL (`chart`, `sort`, `limit`, `position`).
- **Фикс теста:** `order-economics` был привязан к августу при `period=month` — стал независимым от календаря.

### Ключевые находки
- Мойка «Комплекс Базовый» 3000–3600 ₽ против локальных комплексов 800–2200 ₽ — держим цену Казани в Чебоксарах.
- Диагностика электрики 500 ₽ против рынка 800–1500 ₽ — резерв цены.
- Перетяжка руля 1500 ₽ и потолка 5000 ₽ — ниже локального минимума ателье.
- Установка сигнализации 8000 ₽ за работу против 2375–5000 ₽ — нужно разделить на базовую и с автозапуском.
- Между нашим кварцем (1–2 тыс.) и локальной керамикой (7–30 тыс.) пустая ступень — крупнейший резерв маржи.

### Key files
- `lib/marketAnalysis.js` (переписан), `lib/marketBenchmark.js` (новый)
- `controllers/marketAnalysisController.js`, `views/admin/market.ejs`, `public/js/admin-market.js`
- `docs/MARKET_ANALYSIS_2026_09.md` (новый), `docs/MARKET_ANALYSIS_AUTO_INTERIOR_2026.md` (помечен как исторический)
- `tests/market-analysis-ui.test.js`, `tests/market-benchmark.test.js`, `tests/order-economics.test.js`
...
- Срез ручной на одну дату, не live-парсинг; цены «от» ≠ финальная смета.
- Профи.ру / Услугио — частные мастера, нижняя граница рынка.
- Сравнение по базовому классу авто; у конкурентов разбивка 1–5 класс.
- Факт по услугам вне каталога — текстовое совпадение по `name_lc`, ломается при переименовании услуги.
